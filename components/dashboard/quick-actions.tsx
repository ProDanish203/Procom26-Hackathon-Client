        ...
        <div className="grid grid-cols-2 gap-4 w-full">
            {actions.map((action) => (
                <div key={action.label} className="flex flex-col items-center gap-2">
                    <Button
                        size="icon-lg"
                        className="text-black rounded-xl shadow-md hover:scale-105 transition-all duration-300 w-14 h-14 sm:w-16 sm:h-16 bg-gold shadow-gold/30 hover:shadow-gold/40 hover:bg-gold/90"
                    >
                        <action.icon className="h-6 w-6 sm:h-7 sm:w-7" />
                    </Button>
                    <span className="text-xs font-bold text-foreground uppercase">{action.label}</span>
                </div>
            ))}
        </div>
    );
